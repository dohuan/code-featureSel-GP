function [location, features] = extract_features(style,nf,mode,folderpath)
% if mode == 0
%     folderpath = '.\images2\';
% else
...
if strcmp(style,'HISTOGRAM')
    ...
    for index=1:nt
        ...
        %RGB_image = imread([folderpath num2str(index) '-d.png']);
        %Gray_image = rgb2gray(RGB_image);
        %[y,~] = imhist(RGB_image(:,:,1),nf);
        %         gof = createFits(x, counts); % this line fits a multi model Gaussian distribuation on histogram
        features(index,:) = y;
        progressbar(index/nt)
    end
end
